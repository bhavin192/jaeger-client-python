...
def test_large_ids(tracer):
    ...
    assert thrift.id_to_low(trace_id) == 0xffffffffffffffff
    assert thrift.id_to_high(trace_id) == 0xffffffffffffffff

    trace_id = 0xfb34678b8864f051e5c8c603484e57
    span_id = 0x77fd53dc6b437681
    serialize(trace_id, span_id)
    assert thrift.id_to_low(trace_id) == 0x51e5c8c603484e57
    assert thrift.id_to_high(trace_id) == 0xfb34678b8864f0

def test_none_ids():
    assert thrift.id_to_int(None) is None
    assert thrift.id_to_low(None) is None
    assert thrift.id_to_high(None) is None
